...
function AddItems(data) {
    ...
    var AddOption3 = '<div class="MainBodyTextUI"><i class="fas fa-th"></i> Crafting</div>' +
        '<div class="XPShow"><div class="pro"><p class="TextXPUI"><i style="color: black" class="fas fa-pencil-ruler"></i> XP ' + data.xp + '</p></div></div>'
...
function CloseCrafting() {
    $.post("https://qb-crafting/CloseCrafting", JSON.stringify({}));
}
    ...
    if (selectA != null) {
        if (AmountCraft > 0) {
            ...
            if (craftType !== undefined && craftType !== null) {
                $.post("https://qb-crafting/craftTypeStarted", JSON.stringify({ mat: DataItems[selectA], amount: AmountCraft, craftingType: craftType }));
            } else {
                $.post("https://qb-crafting/CraftStarted", JSON.stringify({ mat: DataItems[selectA], amount: AmountCraft }));
            }
        } else {
            var AlertText = "Enter the number !"
            var AlertType = "error"
            $.post("https://qb-crafting/SendAlertcraft", JSON.stringify({ text: AlertText, type: AlertType }));
        }
    } else {
